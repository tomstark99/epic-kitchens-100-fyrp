...
CLASS_COUNTS_DICT = {'verb': 97, 'noun': 300}
...
class EpicActionRecogintionShapleyClassifier:
    ...
    def _get_weights(self):
        if self.task_type:
            train_labels = self.trainloader.dataset.pkl_dict['labels']
            test_labels = self.testloader.dataset.pkl_dict['labels']
            
            train_classes = torch.tensor([label[f'{self.task_type}_class'] for label in train_labels])
            test_classes = torch.tensor([label[f'{self.task_type}_class'] for label in test_labels])
