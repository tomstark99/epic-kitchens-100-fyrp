...
parser.add_argument("mf_features_dir", type=Path, help="Path to features")
...
def main(args):
    ...
    dataset = HDF5Dataset(args.mf_features_dir, args.features_dir)
    dataloader = DataLoader(dataset, batch_size=1)

    n_frames = args.sample_n_frames
    frame_sampler = RandomSampler(frame_count=n_frames, snippet_length=1, test=True)

    v_models = [V_MF(frame_count=i, hidden_layer_size=256, dropout_probability=0.5) for i in range(1,9)]
    n_models = [N_MF(frame_count=i, hidden_layer_size=256, dropout_probability=0.5) for i in range(1,9)]

    assert len(v_models) == len(n_models)

    for i in range(len(v_models)):
        v_models[i].load_state_dict(torch.load(args.checkpoints / f'mf-type=verb-frames={i+1}-batch_size=512-lr=1e-05_hl=256_dcp=0.5_epoch=200.pt'))
        n_models[i].load_state_dict(torch.load(args.checkpoints / f'mf-type=noun-frames={i+1}-batch_size=512-lr=1e-05_hl=256_dcp=0.5_epoch=200.pt'))
